...
usage() {
  echo "usage: run_nrt_path_latency_gate.sh [workspace-root] [headless|gazebo|both]" >&2
...
case "${GATE_MODE}" in
  ...
  *)
    echo "nrt_path_latency_gate: unsupported mode=${GATE_MODE} (expect headless|gazebo|both)" >&2
    exit 64
    ;;
esac
...
if [[ ! -f "${ROS_SETUP}" ]]; then
  echo "nrt_path_latency_gate: missing ROS setup: ${ROS_SETUP}" >&2
  exit 1
fi
if [[ ! -f "${WS_SETUP}" ]]; then
  echo "nrt_path_latency_gate: missing workspace setup: ${WS_SETUP}" >&2
  exit 1
fi
...
if ! command -v ros2 >/dev/null 2>&1; then
  echo "nrt_path_latency_gate: ros2 CLI not available after sourcing setup files" >&2
  exit 1
fi
...
if [[ ! -f "${PROBE}" ]]; then
  echo "nrt_path_latency_gate: missing probe: ${PROBE}" >&2
  exit 1
fi
...
if [[ ! -f "${MODEL_PATH}" ]]; then
  echo "nrt_path_latency_gate: missing xacro model for Gazebo launch" >&2
  exit 1
fi
...
run_probe() {
  local mode="$1"
  local output_log="${LOG_DIR}/nrt_path_latency_${mode}_probe.log"
  set +e
  timeout 90 "${ROKAE_PYTHON_EXECUTABLE:-python3}" "${PROBE}" \
    --mode "${mode}" \
    --namespace /xmate_er3 \
    --remote-ip "${ROKAE_REMOTE_IP:-127.0.0.1}" \
    --local-ip "${ROKAE_LOCAL_IP:-127.0.0.1}" | tee "${output_log}"
...
launch_headless() {
  local launch_log="${LOG_DIR}/nrt_path_latency_headless_launch.log"
  ...
  if [[ ! -x "${runtime_exe}" ]]; then
    echo "nrt_path_latency_gate: missing runtime daemon executable rokae_sim_runtime" >&2
    return 1
  fi
  echo "[nrt_path_latency_gate] launching headless daemon NRT runtime..."
  "${runtime_exe}" \
    --ros-args \
...
launch_gazebo() {
  local launch_log="${LOG_DIR}/nrt_path_latency_gazebo_launch.log"
  echo "[nrt_path_latency_gate] launching Gazebo/JTC diagnostic runtime..."
  GAZEBO_MASTER_URI="${GAZEBO_MASTER_URI:-http://127.0.0.1:11459}" \
...
run_headless_gate() {
  ...
  if ! run_probe headless; then
    echo "nrt_path_latency_gate: FAIL tier=headless classification=strict_dev_latency_failure" >&2
    cleanup_launch
    return 1
  fi
  echo "nrt_path_latency_gate: PASS tier=headless classification=pass"
  cleanup_launch
}
...
run_gazebo_diagnostic() {
  ...
  if run_probe gazebo; then
    echo "nrt_path_latency_gate: PASS tier=gazebo classification=pass"
    cleanup_launch
    return 0
  fi
  echo "nrt_path_latency_gate: DIAGNOSTIC tier=gazebo classification=gazebo_diagnostic_latency" >&2
  cleanup_launch
  return 0
}
