...
namespace RtCompatFields {
/**
 * @brief Compatibility-only RT metadata fields.
 *
 * These fields are additive and do not belong to the official xCore SDK RT field set.
 * They exist solely so the ROS2/Gazebo-backed compatibility facade can expose native
 * scalar/bool RT metadata through the same startReceiveRobotState()/getStateData() path.
 */
constexpr const char *samplePeriod_s = "compat_sample_period_s";
constexpr const char *sampleFresh = "compat_sample_fresh";
}  // namespace RtCompatFields
