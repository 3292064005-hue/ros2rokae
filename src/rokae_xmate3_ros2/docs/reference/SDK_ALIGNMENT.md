...
> 上游事实来源：`docs/reference/xmate6_official_alignment_manifest.json`、compat tests、`cmake/xCoreSDKConfig.cmake.in`  
> 最后校验：2026-04-18
...
- machine-readable manifest：`docs/reference/xmate6_official_alignment_manifest.json`
...
历史上的 `COMPAT_ABI.md`、`API_ALIGNMENT_MATRIX.md`、`XMATE6_OFFICIAL_ALIGNMENT_MATRIX.md` 已并入本页和 `docs/public/COMPATIBILITY.md`，不再保留独立正文。

## 2. public contract 摘要

### install-facing 公开面
- headers：`include/rokae/*`
- targets：`xCoreSDK::xCoreSDK_static`、`xCoreSDK::xCoreSDK_shared`
- canonical identity：`xCoreSDK:xmate6`
...
- `test/harness/check_xmate6_alignment_behaviors.py`：alignment 文档、manifest、CMake labels、执行 gate 入口与关键断言 token 的一致性
- `tools/run_xmate6_alignment_behavior_gate.sh`：在 build-capable 环境中执行 `ctest -L xmate6_alignment` 的强行为验收入口；默认绑定进 `run_quick_gate.sh`、`run_release_gate.sh` 和 L1 acceptance
...
- `MoveSP`、路径录制/回放已纳入 xMate6 public lane 的 NRT 扩展面
